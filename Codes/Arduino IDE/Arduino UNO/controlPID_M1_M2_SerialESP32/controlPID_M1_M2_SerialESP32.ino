...
void EmergencyStop() {
  ...
  Setpoint_M2 = current_position_M2;
  Abs_Setpoint_M2 = abs(current_position_M2);
  
  Serial.println("PARADA DE EMERGENCIA - Motores detenidos");
}
...
void ProcesarComandos() {
  if (newData == true) {
    ...
    // Buscar comando ESTOP (parada de emergencia)
    char* pos_ESTOP = strstr(comando, "ESTOP");
    if (pos_ESTOP != NULL) {
      EmergencyStop();
      return;
    }

    // Si no se encuentra ningún comando válido, mostrar mensaje
    if (pos_V1 == NULL && pos_P2 == NULL && pos_ESTOP == NULL) {
      Serial.println("Comando no reconocido. Use V1:, P2: o ESTOP");
    }
...
void EnviarDatos() {
  ...
  // Enviar datos cada 50ms
  if (currentTime - lastPrintTime >= 50) {
    lastPrintTime = currentTime;
    
    // Calcular posición angular en radianes
    float th2_rad = current_position_M1;  // Ya está en radianes
    
    // Enviar datos en formato para ESP32: "DATA:th2:X.XXXX,lDE:Y.YY"
    Serial.print("DATA:th2:");
    Serial.print(th2_rad, 4);
    Serial.print(",lDE:");
    Serial.println(current_position_M2, 2);
  }
}
